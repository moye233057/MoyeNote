...
if __name__ == '__main__':
    ...
    #     text = f.read()
    text = """
    波士顿温特沃斯理工学院 (WIT) 的四名学生团队创建了一个支持物联网 (IoT) 的智能镜子，以更好地将门诊患者与医疗保健提供者联系起来。
    该镜子专为帮助改进电子病历和提供更准确的患者报告数据而设计，允许用户通过多种方式跟踪他们的健康状况。
    信息可以通过手动输入、链接的可穿戴设备和智能设备以及集成传感器收集。
    每天在方便的时间和地点进行检查，使患者更容易准确记录他们的病情。他们不再需要记住然后回顾性报告他们的健康状况。
    镜子有两种尺寸。一个是全长版本，另一个是较短的药柜尺寸。触摸屏镜子包括面部识别功能、语音命令和摄像头。
    目前是一个原型，镜像的开发计划侧重于数据分析、通信安全和易用性。
    设计师强调需要让广泛的用户可以使用镜子，以使其成为有用的医疗保健工具。
    """
    stop_word = []
